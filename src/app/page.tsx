...
const initialEdges: Edge[] = [{ id: 'e1-2', type: 'step-edge', source: '1', target: '2', animated: true }];
...
export default function App() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 'example-1',
      content: 'Welcome! I\'m your AI assistant. I can help you with various tasks. Feel free to ask me anything!',
      role: 'assistant',
      timestamp: new Date(Date.now() - 1000 * 60 * 5) // 5 minutes ago
    },
    {
      id: 'example-3',
      content: 'Of course! I\'d be happy to help. What kind of project are you working on? I can assist with coding, design, planning, or any other aspects you need help with.',
      role: 'assistant',
  ...
  useEffect(() => {
    if (messages.length > 0) {
      ...
      // Auto-expand when new message arrives
      setIsExpanded(true);
      // Auto-collapse after 3 seconds if not manually expanded
  ...
  return (
    <div className="w-screen h-screen">
      ...
      <motion.div 
        className="fixed bottom-5 left-1/2 -translate-x-1/2 w-[35%] min-w-[400px] max-w-[600px] z-50 flex flex-col gap-0 bg-transparent p-4 rounded-lg"
      ...
      >
        ...
        <motion.div
          onTap={() => messages.length > 0 && setIsExpanded(!isExpanded)}
          className={cn(
            "relative w-full flex justify-center items-center z-20",
            messages.length > 0 ? "cursor-pointer active:cursor-grabbing" : "cursor-not-allowed"
          )}
        ...
        >
          <motion.div 
            className={cn(
              "absolute p-2",
              messages.length > 0 ? "text-gray-400" : "text-gray-600"
            )}
        ...
        <div className="relative z-10">
          <PromptInputBox onSend={onMessageSend} isLoading={isLoading} />
        </div>
      </motion.div>

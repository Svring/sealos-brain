import type { AIProxyNodeData } from "@/components/flow/node/aiproxy/aiproxy-node";
import type { AiproxyColumn } from "@/components/inventory/aiproxy/aiproxy-table-schema";
...
export function transformAIProxyTokensToTable(data: any): AiproxyColumn[] {
  ...
  return tokens.map((token: any) => {
    ...
    const graph = token.labels?.[GRAPH_NAME_LABEL_KEY] || "none";
    ...
    return {
      ...
      charged: `$${(token.used_amount / 100_000 || 0).toFixed(2)}`, // Convert from cents to dollars

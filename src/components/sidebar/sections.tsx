...
export const DashboardSection: React.FC<{ getAccountBalance: () => string }> = ({ getAccountBalance }) => (
  <div className="p-4">
    <h3 className="text-lg font-semibold mb-4">Dashboard</h3>
    <div className="space-y-4">
      <SpotlightCard className="p-4 w-full">
        <h4 className="text-sm font-medium mb-2 text-[var(--heading-text)]">Account Balance</h4>
        <div className="text-2xl font-bold text-[var(--heading-text)]">${getAccountBalance()}</div>
      </SpotlightCard>
    </div>
  </div>
);
...
export const GraphSection: React.FC = () => (
  <div className="p-4">
    ...
    <p className="text-sm text-muted-foreground">Graph visualization coming soon...</p>
  </div>
);
...
export const DevboxSection: React.FC<DevboxProps> = ({ loading, error, data }) => (
  <div className="p-4">
    <h3 className="text-lg font-semibold mb-4">DevBox</h3>
    {loading && <div className="text-sm text-muted-foreground">Loading devbox list...</div>}
    {error && <div className="text-sm text-red-500 mb-4">Error: {error}</div>}
    {data && (
      <div className="space-y-4">
        <div className="text-sm text-green-600">✅ Devbox list loaded successfully!</div>
    ...
    {!loading && !error && !data && (
      <p className="text-sm text-muted-foreground">Click to load devbox list or select a DevBox tool from the navigation menu.</p>
    )}
  </div>
);
...
export const DatabaseSection: React.FC = () => (
  <div className="p-4">
    ...
    <p className="text-sm text-muted-foreground">Database management interface coming soon...</p>
  </div>
);
...
export const AIProxySection: React.FC = () => (
  <div className="p-4">
    ...
    <p className="text-sm text-muted-foreground">AI Proxy configuration and management coming soon...</p>
  </div>
); 
...
function handleError(error: unknown, operation: string): never {
	...
	throw new PostgreSQLError(`PostgreSQL ${operation} failed: ${message}`, operation, error);
}
...
export async function connectPostgreSQL(
	...
	try {
		...
		if (typeof config === 'string') {
			...
			connectionString = process.env.POSTGRESQL_URL || "postgresql://postgres:password@localhost:5432/postgres";
		}
...
export async function query(
  ...
  try {
    ...
    const result = await (database as any).$client.unsafe(sqlQuery, params || []);

    const columns = result.length > 0 && result[0] ? Object.keys(result[0]) : [];
...
export async function update(
  ...
  try {
    ...
    const setClause = updateEntries.map(([col, _], i) => `"${col}" = $${i + 1}`).join(', ');
    const values = Object.values(data) as any[];

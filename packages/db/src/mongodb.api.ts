...
function handleError(error: unknown, operation: string): never {
	...
	throw new MongoDBError(`MongoDB ${operation} failed: ${message}`, operation, error);
}
...
function buildConnectionString(config: MongoConnectionConfig): string {
	...
	console.log(`Built connection string: ${uri.replace(/\/\/[^@]*@/, '//***:***@')}`);
	return uri;
}
...
export async function find(
  ...
  try {
    ...
    const hasMore = options.limit ?
      await collection.countDocuments(query) > (options.skip || 0) + documents.length :
      false;
...
export async function insertMany(
  ...
  try {
    ...
    return {
      insertedId: Object.values(result.insertedIds) as mongoose.Types.ObjectId[],
      acknowledged: result.acknowledged,

...
import { transformRegionUrl } from "@sealos-brain/shared/network/utils";
import { z } from "zod";
import type {
	ClusterResource,
	ComponentSpec,
...
const transformComponents = (resource: ClusterResource) => {
	...
	return componentSpecs.map((spec: ComponentSpec) => {
		...
		const memory = standardizeUnit(
			spec.resources?.limits?.memory || spec.resources?.requests?.memory || "0",
			"memory",
		);
...
// biome-ignore lint/suspicious/noExplicitAny: Generic resource transformation
const transformConnection = async (resources: any) => {
...
// Transform schema - defines transforms for all fields
export const ClusterBridgeTransSchema = z.object({
	name: z.any(),
	uid: z.any(),
	type: z.any(),
	version: z.any(),
	...
	components: z.any().transform(transformComponents),
	connection: z.any().transform(transformConnection),
	backup: z.any(),
	pods: z.any().transform(transformPods).optional(),
});
